...
struct BoardService {
    ...
    static func postBoard(topic: String, place: String, start: String, description: String, photo: Data?, completion: @escaping(Result<(ResponseMsgArr, Int), Error>) -> Void) {
    ...
    // MARK: - 게시글 수정 API 호출 로직
    static func updateBoard(topic: String, place: String, start: String, description: String, photo: Data?, completion: @escaping(Result<(ResponseMsgArr, Int), Error>) -> Void) {
        guard let token = Keychain.read(key: "accessToken") else { return }
        do {
            guard let url = URL(string: "\(Network.url)/board/write") else {
                print("Cannot create URL!")
